	...
	public static class AttributeCopier
	{
									...
									String setterName)
		{
			...
			if (value != null)
			{
				try
				{
					Method method = SampleBean.class.getDeclaredMethod(setterName, String.class);

					if (value.isPresent())
					{
						method.invoke(dest, value.get());
					}
					else
					{
		...
		{
			setAttribute(dest, source, "alpha", "setAlpha");
			setAttribute(dest, source, "bravo", "setBravo");
			setAttribute(dest, source, "charlie", "setCharlie");
		}
	}
